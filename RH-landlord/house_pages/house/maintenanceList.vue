	...
	<view class="page-view">
		<view class="maintenance-item" v-for="(item) in list" :key="item.id">
			...
			<view class="item-foot">
				...
				<u-button :loading="solveLoading" @click="solve(item)" type="primary" shape="circle"
					icon="checkmark-circle" text="解决"></u-button>
			</view>
		</view>
		...
		<u-empty marginTop="40px" mode="order" v-if="list.length===0" text="暂无报修!">
		</u-empty>
	</view>
</template>
		...
		methods: {
			getList() {
				return this.$http.request({
					url: "/api/landlordUser/getMaintenanceListByLandlordId",
					method: "get",
				}).then(({
					status,
					data
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							this.$http.request({
								url: "/api/house/solveMaintenance",
								method: "get",
								data: {
									id
								}
